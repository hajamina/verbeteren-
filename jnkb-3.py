...
# Definieer vaste kleuren voor de drie staten (CA, NY, IL)
state_colors = {
    'IL': 'blue',       # Illinois
...
for idx, row in filtered_data.iterrows():
    # Kleur toewijzen op basis van de staat
    state_color = state_colors.get(row['State'], 'gray')  # Gebruik grijs als back-up kleur
    attrition_color = 'green' if row['Attrition'] == 0 else 'red'  # Groen voor gebleven, rood voor vertrokken
    
    folium.CircleMarker(
        location=[row['latitude'], row['longitude']],
        radius=7,
        color=state_color,    # Buitenrand van de marker
        fill=True,
        fill_color=attrition_color,  # Vulkleuren afhankelijk van attritie-status
        fill_opacity=0.7,
...
# Legenda toevoegen voor staten en attritie-status
legend_html = '''
     ...
     <b>Legenda:</b><br>
     <i class="fa fa-circle" style="color:blue"></i> Illinois (IL)<br>
     <i class="fa fa-circle" style="color:orange"></i> California (CA)<br>
     <i class="fa fa-circle" style="color:purple"></i> New York (NY)<br>
     <i class="fa fa-circle" style="color:green"></i> Gebleven<br>
     <i class="fa fa-circle" style="color:red"></i> Vertrokken<br>
     </div>
'''
...
# Samenvatting van de geselecteerde data in het Nederlands
st.write(f"Aantal werknemers in geselecteerde staten: {len(filtered_data)}")
st.write(f"Vertrekkende werknemers: {len(filtered_data[filtered_data['Attrition'] == 1])}")
st.write(f"Gemiddelde aantal jaren in dienst voor vertrekkende werknemers: {filtered_data[filtered_data['Attrition'] == 1]['YearsAtCompany'].mean():.2f}")
st.write(f"Meest voorkomende afdeling bij vertrekkende werknemers: {Counter(filtered_data[filtered_data['Attrition'] == 1]['Department']).most_common(1)[0][0]}")
